 ...
 * BLE data when connected, fake otherwise). Gear comes from the Room-backed
 * bike profile, bottom-row cards are individually toggleable via the
 * Appearance screen, Start/Stop Ride persists a completed
 * [com.voidroot.bikeos.data.repository.RideSession] to Room, and the Bike
 * Control Panel's light toggles send real Control Service commands to the
 * ESP32 (which now really drives the light MOSFETs). Mode/Gear can also be
 * changed from the physical handlebar buttons - see [DashboardViewModel]'s
...
private fun TopStatusRow(
    ...
    Row(
        ...
        GlassCard { Text("$batteryPercent%", style = MaterialTheme.typography.labelSmall, color = BikeTextSecondary) }
        GlassCard { Text(currentTime, style = MaterialTheme.typography.labelSmall, color = BikeTextSecondary) }
        GlassCard(modifier = Modifier.clickable(onClick = onToggleRide)) {
            Text(
                text = if (isRideActive) "■ Stop Ride" else "▶ Start Ride",
                style = MaterialTheme.typography.labelSmall,
                color = if (isRideActive) BikeDanger else BikeAccent
            )
        }
    }
}
...
private fun BottomInfoRow(
    ...
    Row(
        ...
        if (WidgetKeys.DISTANCE in enabledWidgetKeys) {
            GlassCard {
                Column {
                    Text("Distance", style = MaterialTheme.typography.labelSmall, color = BikeTextSecondary)
                    Text(
        ...
        if (WidgetKeys.CALORIES in enabledWidgetKeys) {
            GlassCard {
                Column {
                    Text("Calories", style = MaterialTheme.typography.labelSmall, color = BikeTextSecondary)
                    Text("$calories kcal", style = MaterialTheme.typography.titleMedium, color = BikeTextPrimary)
                }
            }
        }
        if (WidgetKeys.CADENCE in enabledWidgetKeys) {
            GlassCard {
                Column {
                    Text("Cadence", style = MaterialTheme.typography.labelSmall, color = BikeTextSecondary)
                    Text("$cadenceRpm rpm", style = MaterialTheme.typography.titleMedium, color = BikeTextPrimary)
                }
            }
        }
        if (WidgetKeys.GEAR in enabledWidgetKeys) {
            GlassCard {
                Column {
                    Text("Gear", style = MaterialTheme.typography.labelSmall, color = BikeTextSecondary)
                    Text("$frontGear x $rearGear", style = MaterialTheme.typography.titleMedium, color = BikeTextPrimary)
                }
            }
        }
    }
